...
def demo_random_instance_service():
    """Demonstrate service discovery for multiple instance"""
    print("\n" + "="*60)
    print("SERVICE DISCOVERY DEMO")
    print("="*60)
    ...
    for x in range(10):
        try:
            response = requests.get(f"{registry_url}/discover/user-service")
            if response.status_code == 200:
                data = response.json()
                print(f"📋 Total instances registered: {data['count']}")
            chosen = random.choice(data['instances'])
            print(f"Selected Instance: {chosen['address']}")
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "demo-rand":
        demo_random_instance_service()
